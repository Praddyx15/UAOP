...
**Version 0.1.4 · 2026-07-07 · 19 ADRs, 5 reviews (R1–R5) to date.**
...
4. **Prompt-hygiene rule for AI-first development (R5/F34):** never paste Tier 3 source into a code-generation prompt — "Claude, refine this ArduPilot function for our codebase" produces a derivative work with extra steps. Describe the observed behavior in words or via the protocol specification, then implement fresh. This rule is in CLAUDE.md/AGENTS.md as a hard rule because the AI-first workflow makes this the single most likely accidental-contamination path.
...
| F34 | Weakness / AI-first process | The AI-assisted workflow makes license contamination *easier than ever*: pasting GPL source into a generation prompt and asking for a "refined" version produces a derivative work that looks original. No existing rule addressed this. | Prompt-hygiene rule in ADR-0019 §4 + hard-rule line added to CLAUDE.md/AGENTS.md. |
