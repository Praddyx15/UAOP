...
- 🟡 **M0.6** compose.yaml + setup.sh authored, `docker compose config` valid; live `up --wait` verification pending Docker engine availability (first-run Docker Desktop dialog needs a human click).
...
- **M0 Definition of Done: fully met except the M0.6 live-infra check**, which only needs your one-time Docker Desktop click — nothing else is blocking M1.
